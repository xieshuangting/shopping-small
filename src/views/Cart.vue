							...
							<li v-for="item in cartList">
								<div class="cart-tab-1">
									<div class="cart-item-check">
										<a htef='javascript:;' class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked == '1'}" @click="editCart('checked',item)">
...
// import {currency} from './../util/currency'局部
...
export default {
  ...
  methods:{
  	...
  	delCartConfirm(productId,delNum){
  	...
  	delCart(){
  		this.$axios.post('/users/cartDel',{
  			...
  			if(res.status == '0'){
  				this.modalConfirm = false;
  				this.init();
  				this.$store.commit('updataCartCount',-this.delNum);
  			}
  		})
  	},
  	editCart(flag,item){
